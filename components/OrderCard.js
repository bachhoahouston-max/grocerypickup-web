...
const getProductCombo = (product) =>
    product?.combo_id && typeof product.combo_id === "object" ? product.combo_id : null;
...
const StatusBadge = ({ booking }) => {
    ...
    const map = {
        Completed: { bg: "bg-green-100", text: "text-green-700", label: t("Order Delivered") },
        Pending: { bg: "bg-amber-100", text: "text-amber-700", label: booking?.isReady ? t("Order Ready") : t("Order Preparing") },
        "Return Requested": { bg: "bg-blue-100", text: "text-blue-700", label: t("Order Return Requested") },
        Return: { bg: "bg-blue-100", text: "text-blue-700", label: t("Order Returned") },
        Cancel: { bg: "bg-red-100", text: "text-red-700", label: t("Order Cancelled") },
        Shipped: { bg: "bg-green-100", text: "text-green-700", label: t("Order Shipped") },
        Preparing: { bg: "bg-green-100", text: "text-green-500", label: booking?.isReady ? t("Order Ready") : t("Order Preparing") },
        Driverassigned: { bg: "bg-green-100", text: "text-green-500", label: t("Driver Assigned") },
        "Out for Delivery": { bg: "bg-green-100", text: "text-green-700", label: t("Out for Delivery") },
    };
    const b = map[booking?.status];
    if (!b) return null;
    return (
        <span className={`px-3 py-1.5 ${b.bg} ${b.text} rounded-full text-xs font-medium whitespace-nowrap`}>
            {b.label}
        </span>
    );
};
...
const DeliveryTypeLabel = ({ booking }) => {
    const { t } = useTranslation();
    if (booking?.isShipmentDelivery) return <p className="text-gray-700 font-medium text-sm">{t("Shipment Delivery")}</p>;
    if (booking?.isLocalDelivery) return <p className="text-gray-700 font-medium text-sm">{t("Local Delivery")}</p>;
    if (booking?.isDriveUp) return <p className="text-gray-700 font-medium text-sm">{t("Curbside Pickup")}</p>;
    if (booking?.isOrderPickup) return <p className="text-gray-700 font-medium text-sm">{t("In-store Pickup")}</p>;
    ...
    return (
        <div className="rounded-xl overflow-hidden border border-green-200 mb-1">
            ...
            <div className="px-3 py-2 bg-green-50">
                <p className="text-green-800 text-xs font-semibold">{combo.promo_text}</p>
                <div className="flex flex-wrap gap-2 mt-1.5">
                    {combo.free_product?.map((fp, i) => (
                        <span key={i} className="inline-flex items-center gap-1 bg-green-100 text-green-700 text-xs font-bold px-2.5 py-1 rounded-full border border-green-200">
                            🎁 FREE · {fp.slot?.unit}
                            <span className="font-normal text-green-500">(${fp.slot?.our_price?.toFixed(2)} value)</span>
                        </span>
                    ))}
                </div>
            </div>
...
const NormalProductRow = ({ product, index, total, booking, lang, router }) => (
    ...
    >
        ...
        <div className="text-right flex-shrink-0 pr-1">
            <p className="text-sm font-bold text-gray-800">${parseFloat(product.price || 0).toFixed(2)}</p>
        </div>
    </div>
);
...
const FreeProductRow = ({ product, index, total, booking, lang, router }) => (
    ...
    >
        ...
        <div className="text-right flex-shrink-0 pr-1">
            <p className="text-xs text-gray-400 line-through">${parseFloat(product.price || 0).toFixed(2)}</p>
...
const ComboPriceSummary = ({ booking }) => {
    ...
    const normalProducts = booking.productDetail.filter((p) => !getProductCombo(p));
    ...
    const normalSubtotal = normalProducts.reduce((s, p) => s + parseFloat(p.price || 0), 0);
    const comboRetail = comboProducts.reduce((s, p) => s + parseFloat(p.price || 0), 0);
    const freeRetail = combos.reduce((s, c) => s + c.free_product.reduce((fs, fp) => fs + parseFloat(fp.slot?.our_price || 0), 0), 0);
        ...
        <div className="mt-3 rounded-xl border border-green-200 bg-green-50 px-4 py-3 space-y-1.5">
            ...
            {normalProducts.length > 0 && (
                <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-gray-700 font-medium">${normalSubtotal.toFixed(2)}</span>
                </div>
            )}
            ...
            <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-gray-400 line-through">${(comboRetail + freeRetail).toFixed(2)}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
                <span className="text-gray-500 flex items-center gap-1">
                    🎁 {t("Free items value")}
                </span>
                <span className="text-green-600 font-semibold">-${freeRetail.toFixed(2)}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
...
// ─── Free Item Row (fully populated from combo_id.free_product[]) ─────────────
const FreeItemRow = ({ fp, lang, isLast }) => {
    ...
    const name = lang === "en"
        ? product?.name
        : (product?.vietnamiesName || product?.name);

    return (
        <div className={`flex items-center p-2 relative bg-green-50/50 ${!isLast ? "border-b border-gray-200" : ""}`}>
...
const ProductList = ({ booking, lang, router }) => {
    ...
    // ── Section 1: Normal items ─────────────────────────────────────────────────
    if (normalProducts.length > 0) {
        // Section divider — only shown in mixed orders
        if (isMixedOrder) {
            rendered.push(
                <div key="divider-normal" className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 border-b border-gray-200">
                    <span className="text-xs font-bold text-gray-500 uppercase tracking-wide">Regular Items</span>
                    <span className="ml-auto text-xs text-gray-400">{normalProducts.length} item{normalProducts.length > 1 ? "s" : ""}</span>
                </div>
            );
        }
    ...
    if (comboProducts.length > 0) {
        ...
        if (isMixedOrder) {
            rendered.push(
                <div key="divider-combo" className="flex items-center gap-2 px-3 py-1.5 bg-green-700 border-b border-green-600">
                    <span className="text-xs font-bold text-white uppercase tracking-wide">🎁 Combo Deal</span>
                    <span className="ml-auto text-xs text-green-200">
                        {comboProducts.length} item{comboProducts.length > 1 ? "s" : ""} + free gift{comboProducts.length > 1 ? "s" : ""}
                    </span>
                </div>
            );
        }
...
const OrderCard = ({
    ...
    return (
        <div className="bg-white rounded-lg shadow-md border border-gray-200 hover:border-gray-300 transition-all mb-2 p-1">
            ...
            {/* ── Header ─────────────────────────────────────────────────────────── */}
            <div className={`p-3 md:p-4 rounded-t-lg ${hasCombo ? "bg-green-50" : "bg-gray-50"}`}>
                <div className="space-y-3 md:space-y-0">

                    {/* Mobile header */}
                    <div className="block md:hidden space-y-3">
                        <div className="flex justify-between items-center">
                            <div className={`text-white rounded-full h-8 w-8 flex items-center justify-center font-semibold text-sm flex-shrink-0 ${hasCombo ? "bg-green-700" : "bg-[#2E7D32]"}`}>
                                {index + 1}
                            </div>
                        ...
                        <div className="space-y-0.5">
                            ...
                            <p className="text-sm font-medium text-gray-800">{t("Order Id")}: {booking.orderId || 1}</p>
                        </div>
                    ...
                    <div className="hidden md:flex justify-between items-center">
                        <div className="flex items-center space-x-3">
                            <div className={`text-white rounded-full h-10 w-10 flex items-center justify-center font-semibold ${hasCombo ? "bg-green-700" : "bg-[#2E7D32]"}`}>
                                {index + 1}
                            </div>
                            <div>
            ...
            {(booking?.SecretCode || booking?.isShipmentDelivery || booking?.trackingNo) && (
                <div className="p-4 border-gray-200 bg-gray-50">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {booking?.SecretCode && (booking?.status === "Pending" || booking?.status === "Preparing") && (
                            ...
                            <div className="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p className="text-base font-medium text-gray-800">
                                        {formatDate2(new Date(new Date(booking.createdAt).setDate(new Date(booking.createdAt).getDate() + 7)))} 11 PM
                                    </p>
                                </div>
                            </div>
                        )}
            ...
            <div className="px-4 py-3 bg-white border-b border-gray-200">
                <div className="flex flex-wrap gap-2 justify-end">
                    {(() => {
                        const diff = (new Date() - new Date(booking.createdAt)) / (1000 * 60);
                ...
                <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50 px-4">
                    <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-gray-700 font-medium mb-1">{t("Car Brand")}</label>
                                <input type="text" value={carBrand} onChange={(e) => setCarBrand(e.target.value)} required
                            ...
                            <div>
                                <label className="block text-gray-700 font-medium mb-1">{t("Car Color")}</label>
                                <input type="text" value={carColor} onChange={(e) => setCarColor(e.target.value)} required
                                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500 text-black"
                                    placeholder={t("Enter Car color")} />
                            </div>
                            <div>
                                <label className="block text-gray-700 font-medium mb-1">{t("Parking Pickup Spot")}</label>
                                <select value={parkingNo} onChange={(e) => setParkingNo(e.target.value)}
                                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500 text-black">
            ...
            <div className={expandedBookingId === booking._id ? "hidden" : "block p-4"}>

                {/* Product rows — smart rendering per product */}
                <ProductList booking={booking} lang={lang} router={router} />

                {/* Combo price summary (only shown if order has combo products) */}
